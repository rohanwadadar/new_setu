...
//convert jsx to html
// Phase 2: Generate physical HTML folders and files for each route
async function prerenderHTML(pages) {
    // Check if the base build exists before proceeding
    ...
    for (const p of pages) {
        ...
        // Save the modified HTML to its specific directory
        fs.writeFileSync(path.join(outDir, 'index.html'), html);
    }

    // System files
    fs.writeFileSync(path.join(DIST_DIR, '.nojekyll'), '');
    // Create necessary fallback files for hosting
